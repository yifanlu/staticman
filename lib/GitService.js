...
class GitService {
  ...
  writeFile (filePath, data, branch = this.branch, commitTitle = 'Add Staticman file', commitAuthor = '', commitEmail = '') {
    return this._commitFile(filePath, Buffer.from(data).toString('base64'), commitTitle, branch, commitAuthor, commitEmail)
  }

  writeFileAndSendReview (filePath, data, branch, commitTitle = 'Add Staticman file', reviewBody = '', commitAuthor = '', commitEmail = '') {
    return this.getBranchHeadCommit(this.branch)
      .then(sha => this.createBranch(branch, sha))
      .then(() => this.writeFile(filePath, data, branch, commitTitle, commitAuthor, commitEmail))
